...
class cmtCtrl {
    ...
    function addPublicCmt($content, $userId, $magazineId) {
        require_once './DBConnect.php';

            $sql = "insert into comments(content, userId, magazineId, cmtType, cmtDate) values(?,?,?,2,now())";
            $stmt = $conn->prepare($sql);
            $stmt->bind_param("sss", $content, $userId, $magazineId);
            $stmt->execute();
            if (mysqli_error($conn)) {
